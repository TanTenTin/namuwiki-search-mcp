...
>
> ⚠️ **`ADMIN_API_TOKEN`은 운영에서 사실상 필수다.** `REQUIRE_API_KEY=true`이면 모든 비-`/health`
> 요청에 키가 필요한데, 이 값이 비면 `/admin/keys`가 비활성화돼 **키를 발급할 수단이 없어 API 전체가 401로 잠긴다.**
...
>
> ⚠️ **신규 서버이거나 `namu-data` 볼륨이 비었으면 `/search`가 조용히 빈 결과를 반환한다.** 위 색인을
> 반드시 실행한다(볼륨 유실·재생성 시에도 동일). 또 자동 배포(`docker compose up`)는 진행 중인 색인을
> 컨테이너 재생성으로 중단시키므로, **색인이 도는 동안에는 push/배포를 하지 않는다.**
